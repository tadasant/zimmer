...
class ProcessTerminationServiceTest < ActiveSupport::TestCase
  ...
  # === Real-process tests: the zombie leak behind #273 ===
  #
  # These use a REAL child because the defect only exists against real kernel
  # semantics: Process.kill(0, pid) keeps succeeding for an unreaped child, so
  # every liveness check reports "still running" for a process that already died.
  # A MockProcessManager cannot reproduce that.

  test "terminating a real child reaps it instead of leaving a zombie behind" do
    pid = Process.spawn("sleep", "30", pgroup: true)
    service = fast_service_for(pid)

    result = service.terminate

    assert_equal :terminated, result.status,
      "a child that died on SIGTERM must not be reported as 'could not be terminated'"
    assert_raises Errno::ECHILD, "the terminated child must have been reaped, not left defunct" do
      Process.waitpid(pid, Process::WNOHANG)
    end
    assert_equal "", `ps -o stat= -p #{pid}`.strip, "pid #{pid} is still in the process table"
  ensure
    cleanup_child(pid)
  end

  test "terminating a real child leaves nothing for ZombieReaperJob to collect" do
    pid = Process.spawn("sleep", "30", pgroup: true)

    fast_service_for(pid).terminate

    snapshot = ZombieChildScanner.new.snapshot
    assert_not_includes snapshot.zombie_child_pids, pid
  ensure
    cleanup_child(pid)
  end

  test "still reports error when the process survives and is not ours to reap" do
    pid = 999_123
    manager = MockProcessManager.new
    manager.running_hook = ->(_pid) { true }
    manager.wait_hook = ->(_pid, _flags) { raise Errno::ECHILD }

    service = create_service_with_existing_process(pid: pid, process_manager: manager)
    service.define_singleton_method(:sleep) { |_seconds| nil }

    result = service.terminate

    assert_equal :error, result.status,
      "a process that is genuinely still running must still report failure"
  end

  private

  # A service whose backoff sleeps are collapsed, so the real SIGTERM -> SIGKILL
  # ladder runs against a real process without the ~15s of real waiting.
  def fast_service_for(pid)
    service = ProcessTerminationService.new(
      process_pid: pid,
      process_manager: SystemProcessManager.new
    )
    service.define_singleton_method(:sleep) { |_seconds| Kernel.sleep(0.005) }
    service
  end
