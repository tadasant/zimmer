...
class GateDecisionTest < ActiveSupport::TestCase
  ...
  test "feedback is not cascaded away with its decision" do
    ...
    # Nothing can destroy a GateDecision, so a `dependent: :destroy` cascade would
    # be unreachable today and a live path to deleting human feedback the moment
    # the parent's guard was ever relaxed.
    assert_equal :restrict_with_exception,
                 GateDecision.reflect_on_association(:feedbacks).options[:dependent]
  end
  ...
  test "recent_first breaks a same-day tie by insertion order" do
    ...
    assert_equal [ same_day_second.id, same_day_first.id, older.id ],
                 GateDecision.for_surface("zimmer").recent_first.pluck(:id)
  end
