...
Zimmer's own catalog ships nine roots. `general-agent` is the catch-all default
(`AgentRootsConfig::DEFAULT_ROOT`); `zimmer` is the root for working on Zimmer itself. Four
`catalog-mgmt-*` roots are *subagent* roots — not user-invocable, spawned as phases by the
`catalog-management` lead root.
