...
**It wires the context deliberately.** Which skills, which MCP servers, which repo-specific
guidance the agent gets is not "whatever is in your `~/.claude` directory today" — it's
resolved from a versioned catalog per session. See [AIR](/air/overview/).

**It closes the loop.** The default goal doesn't just say "write the code." It says: open a
PR, wait for CI, confirm CI is green, run a review, address the review, and only then come
back to the human. See [goals](/sessions/goals/).
...
[known limitation](/limitations/#the-web-ui-has-no-authentication-at-all): the entire web
