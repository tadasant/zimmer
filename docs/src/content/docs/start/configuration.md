...
| `AIR_CATALOG_REF` | Staging-only catalog pinning: rewrites every `github://tadasant/zimmer-catalog/…` URI in the catalog to pin that ref. The in-image catalog declares none, so with the default catalog this pins nothing and staging says so at boot — it bites only on an `AIR_CONFIG` catalog with `github://` sources |
