...
| `SUPERVISOR_PASSWORD` | The HTTP Basic realm in front of `/supervisor`. **Fails closed** — unset means every Administrate dashboard returns 401, including for you. Optional `SUPERVISOR_USERNAME` defaults to `supervisor` | ❌ — seed it into your deploy secrets, then add it to `env.secret` in `config/deploy.*.yml` |
