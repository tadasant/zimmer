...
| `SUPERVISOR_PASSWORD` | The HTTP Basic realm in front of `/supervisor`, the API keys page (`/settings/api_keys`), **and the mutating `POST /health/*` maintenance actions**. **Fails closed** — unset or blank means every Administrate dashboard, the API keys page, *and* the health page's Clean Up / Retry / Trash / Halt queues / Re-arm buttons return 401, including for you. The read-only health dashboard, `/up`, and `POST /health/exit_queue_recovery_mode` stay open regardless, and every gated action has a `POST /api/v1/health/*` sibling behind `API_KEYS`. Optional `SUPERVISOR_USERNAME` defaults to `supervisor` | ⚠️ — the production destination maps the name (`.kamal/secrets.production`, and `env.secret` in `config/deploy.production.yml`), but Kamal can only pass a value the deploy environment already holds, and a value that never arrives is **silent**: `Kamal::Secrets#fetch` raises only when the mapping line is missing, so an unset `PROD_SUPERVISOR_PASSWORD` resolves to blank, the deploy goes green, and the realm stays shut. Set it as a secret on the repo that runs the deploy and name it in every place that workflow enumerates secrets; a `: "${VAR:?}"` assert in the deploy step is what makes a missing one fail loudly. Staging maps neither, so the realm is shut there. A self-hosted deploy seeds its own value and adds the name to `env.secret` |
...
| `SLACK_TRIGGER_INGEST_MODE` | how Slack triggers learn about messages: `poll` (default) or `webhook_with_poll_fallback`, which also fires them from Slack Events API deliveries to `POST /webhooks/slack`. See [Slack Events API delivery](/sessions/triggers/#slack-events-api-delivery) | not set; Slack is polled |
| `GITHUB_TRIGGER_INGEST_MODE` | how `github_issue` triggers learn about new issues: `poll` (default) or `webhook_with_poll_fallback`, which also fires them from GitHub webhook deliveries to `POST /webhooks/github`. See [GitHub webhook delivery](/sessions/triggers/#github-webhook-delivery) | not set; GitHub is polled |
| `GITHUB_WEBHOOK_SECRET` | the secret set on the GitHub webhook, which every delivery's `X-Hub-Signature-256` is verified against. Without it `/webhooks/github` answers 404 whatever the mode | via `mcp_secrets`; ENV is the fallback |
| `SLACK_SIGNING_SECRET` | the Slack app's signing secret, which every Events API delivery is verified against. Without it `/webhooks/slack` answers 404 whatever the mode | via `mcp_secrets`; ENV is the fallback |
