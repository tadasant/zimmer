...
code — not the old docs, which were themselves often wrong.

It exists because a docs site that quietly omits the broken parts is marketing. Each item names a file
so you can go look. Items marked **🔴** would bite a new operator immediately.
...
**`config/deploy.production.yml` does not exist in this repo.** The Terraform compose mounts **no
volumes at all** on the `app` service — not for clones, not for `~/.claude` (the shared credentials file
the entire account-rotation system depends on), not for `~/.config/gh`, not for `~/.local` (where the
entrypoint's background `claude update` writes), not `/var/run/docker.sock` (despite `DockerCleanupJob`
needing it).
...
**Unclear / needs confirmation:** whether the Redis client tolerates this or silently uses db 0.
...
in the first ~30 seconds after a container boot silently use the **old** CLI and Chromium.
...
The Administrate admin panel exposes `claude_accounts` (whose `oauth_config` JSONB holds **plaintext
Anthropic and OpenAI access and refresh tokens**), `mcp_oauth_credentials`, `x_oauth_credentials`, and
`runtime_login_attempts` — as *editable* resources, to anyone who can reach the host.

The security model is entirely "put it on a tailnet," and Zimmer's Terraform does close port 80 at the
DO firewall. But any deployment that exposes port 80 hands an anonymous visitor your refresh tokens.

There are also six `# TODO: Add proper authorization checks` comments in `sessions_controller.rb`
...
database access controls."* The unauthenticated admin panel bypasses those.
...
**This has already caused an outage.** When Claude Code's wording changed, account rotation silently
stopped firing: the session fell through to the transient-rate-limit path, retried six times against an
...
token ⇒ the credential silently becomes single-use and dies with no way to refresh.
...
Nearly every callback is wrapped in a bare `rescue` that logs and swallows, so cleanup can silently not
...
pool — so throttling means *no* Slack polling, and ticks are silently dropped.
