...
### Six states still hold a queued message on an idle session
...
`EnqueuedMessageDrainJob` refuses to deliver in six states, and in each the message waits for that
...
with a retry already scheduled, and — for a session resting in `waiting` — never started, held at
the spot gate, or paused in the spot queue. Each is the right call: delivering would spawn a second
process against one clone, burn the message on the wall that caused the park, hand it to a fresh
start that discards it, or churn its position against a gate that is already going to run it. Each
ends in the message going out on the turn that follows. But a park or a hold that never clears holds
the message indefinitely, and nothing says so.
...
queued message is still undelivered when the window closes — the six states above — **does** wake
its watchers. That is deliberate: nothing re-emits this event, so suppressing there would lose the
wake rather than delay it. The consequence is that those six states now also mean a watcher
