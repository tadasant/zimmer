...
| **Generated status** | The cached blurb an agent wrote when the session last came to rest. It opens with a one-sentence recommendation of what to do, is rendered as inline markdown so its links are clickable, and is clamped to three lines with the full text on hover. Under it, every row carries a freshness chip: **Up to date** (green) when the summary was written from the session's latest message, **Stale** (amber) with how many messages have landed since, or **Regenerating** (indigo) while a new one is being written, next to how long ago the shown one was written. Staleness is by **message count**, not by age: a summary rots because the session said something new, not because time passed. |
