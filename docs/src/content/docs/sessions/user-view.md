...
| **Generated status** | The cached blurb an agent wrote when the session last came to rest. It opens with a one-sentence recommendation of what to do, is rendered as inline markdown so its links are clickable, and is clamped to three lines with the full text on hover. Under it, every row carries a freshness chip: **Up to date** (green) when no messages have landed since the summary was written, **Stale** (amber) with how many have, or **Regenerating** (indigo) while a new one is being written, beside how long ago the shown one was written. The chip is as of page load: a summary that finishes generating does not update the row until you reload. A failed regeneration is not shown on the row; the session's Status panel says so. Staleness is by **message count**, not by age: a summary rots because the session said something new, not because time passed. |
