...
- **A failed model call is reported, not retried.** `pi -p` exits 0 even when the model call
  ...
  turn being reported as a successful pause, but the three recovery paths behind
  `context_length_error?`, `api_error_for_retry?` and `auth_recovery_needed?` are Claude-shaped
  and all three answer `false`. So a Pi provider failure is failed and named rather than retried.
  Making those services transcript-format-agnostic is its own piece of work
