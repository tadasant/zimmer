...
- **A transient provider failure is retried, and a quota wall parks; auth and context length are terminal.** `pi -p` exits 0
  ...
  OpenRouter and the characterization ran against an OpenAI-dialect stub. A 402, or a 429 worded
  the way Pi itself declines to retry (`insufficient_quota`, "quota exceeded", …), is a quota wall:
  the session parks and a timed re-check wakes it (`ProviderQuotaWallPark` — 15 minutes, doubling
  to every 8 hours, for up to seven days), spending no retry budget and paging nobody. A 401/403 and
  a 4xx rejection (context-window refusals among them) are
