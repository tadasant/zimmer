...
### Absence is only an answer when capture could have fired

An empty record has two causes that mean opposite things, and for a while it had one rendering:

* **Zimmer looked and no named human spoke.** An affirmative absence, and the answer a gate is
  entitled to act on.
* **Zimmer could not have seen the human even if they spoke**, because the channel they spoke on is
  not instrumented in this deployment.

The second is not hypothetical. Slack user IDs ship empty (see below) and are filled in at
`/supervisor/users`; until they are, every Slack-origin session renders exactly like a hierarchy
nobody ever spoke into — which is what
[#658](https://github.com/tadasant/zimmer/issues/658) found on a session whose instruction was
sitting in a Slack thread three minutes before it was spawned.
...
| --- | --- |
| A human message is on the record | the entries, as before |
| Empty, every channel instrumented | *No message anywhere in this hierarchy was authored by a named human. Every input channel this hierarchy came in through was instrumented, so this is an affirmative absence…* |
| Empty, a channel uninstrumented | a `**Capture is NOT configured for …**` bullet naming the channel, the sessions and the fix — and *Zimmer captured no human message anywhere in this hierarchy, AND capture is not configured for Slack … Read this as **the check could not be established**, NOT as "no human spoke".* |

The third row deliberately does **not** contain the words *"No message anywhere in this hierarchy was
authored by a named human"*. Agent policy artifacts key on that exact sentence to separate "no human
spoke" (an answer) from "the mechanism is absent" (no answer), so the gap case must not match it.

The bullet is rendered on a non-empty record too: a hierarchy can hold web-UI messages while being
...
**Over REST.** `GET /api/v1/sessions/:id` returns `session_hierarchy`, `human_messages` and
`human_message_capture_gaps` as top-level keys beside `session`, never inside it — `session` means one shape on every response that
