...
| Parameter | Meaning |
| --- | --- |
| `message` | What the child is telling its parent. Bounded so that it plus the framing Zimmer wraps around it fit inside `PROMPT_MAX_LENGTH`; the 422 names the room left |
...
Three refusals, each with something the child can do about it:
