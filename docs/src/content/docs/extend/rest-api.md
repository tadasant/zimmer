...
| `POST` | `/sessions/:id/sleep` | `needs_input` → sleeps; `running` → sets `pending_sleep`. Stamps `metadata["deliberate_sleep_at"]`, because this is the one route into `waiting` that arms nothing: without the marker [`StrandedSleepSweepJob`](/operate/background-jobs/) would read the session as stranded and wake it. Any resume or restart clears it |
