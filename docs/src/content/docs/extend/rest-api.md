...
| `POST` | `/sessions/:id/restart` | clears stale retry metadata and re-queues the job; re-runs the whole setup pipeline if setup never finished (a failed clone, say), and the replacement first turn carries the session's [stored images and files](/limitations/#a-stranded-waiting-session-is-only-rescued-if-it-never-started) |
