...
| `POST` | `/sessions/:id/regenerate_status_summary` | → 202. Queues a forced rewrite of the [Status summary](/sessions/status-summary/); it forks the session and spends an agent turn, so it is asynchronous and must not be polled. An archived session is a normal candidate — the fork needs the session's clone, not its status — but once that clone has been reclaimed this is a 422 with the reason rather than a 202 for work that cannot run |
