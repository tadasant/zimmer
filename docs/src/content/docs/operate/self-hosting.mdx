...
- **Nothing is encrypted at rest, and there is no second wall.** OAuth tokens and secrets are plaintext columns. The admin panel that renders them, the API keys page, and the mutating `POST /health/*` maintenance actions authenticate nobody, like the rest of the web UI, so the tailnet perimeter is the only wall, and agent sessions on the box are already inside it. Don't expose port 80. See [Auth overview](/auth/overview/).
...
- [Auth architecture](/auth/overview/) — how credentials flow, and why the tailnet is the only wall in front of `/supervisor`.
