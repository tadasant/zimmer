...
- **Replace `admin_ssh_pubkeys` before your first deploy.** `deploy-staging.yml` does `cp staging.tfvars.example staging.tfvars` *verbatim*, so whatever key sits in the committed example is what gets authorized for `root` on **your** droplet. The keys it ships with are the author's — Zimmer's own operator identity, the orchestrator that drives the fleet, and the author's break-glass laptop key. A public key is not a secret, but leaving one of those in place authorizes **someone else** for root on your box. Replace the list with your own key, or set it to `[]`, and edit `staging.tfvars.example` itself (`staging.tfvars` is gitignored and would be overwritten by that `cp`). See [Operator keys](/operate/ssh-access/#operator-keys).
