...
10. **Log the instance in — Git first.** The app is up, but it can't do anything useful yet: an agent that can't authenticate to GitHub can't clone, can't push a branch, and can't open a PR. These are interactive OAuth device flows, so they can't be baked into a secret — you run them once, by hand, inside the container:
    ...
    This is **once, not per deploy**: `gh`'s token lands in `~/.config/gh`, Claude's in `~/.claude`, and Codex's in `~/.codex` — all durable named volumes (`gh_config`, `claude_home`, `codex_home`) shared by `web` and `worker` and re-attached across Kamal container swaps. Confirm with `gh auth status`, and read the Claude Code tile on the UI's CLI status page for the rest (`CliStatusService` refreshes it every two minutes). Do **not** reach for `claude whoami` — `whoami` is not a subcommand, so the CLI bills the word as a prompt and answers it with a whole agent turn.
