...
- **Nothing is encrypted at rest, and the second wall covers three surfaces.** OAuth tokens and secrets are plaintext columns. The admin panel that renders them, the API keys page, and the mutating `POST /health/*` maintenance actions all sit behind an HTTP Basic realm keyed on `SUPERVISOR_PASSWORD`, which fails closed — unset or blank means those return 401, so you have to set it before any of them opens for you. That is one shared credential in front of three surfaces, not a login system. The rest of the app authenticates nobody, so the tailnet perimeter is still the wall. Don't expose port 80. See [Auth overview](/auth/overview/).
