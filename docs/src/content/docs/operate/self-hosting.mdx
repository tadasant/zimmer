...
- **Set `admin_ssh_pubkeys` before your first deploy.** `deploy-staging.yml` does `cp staging.tfvars.example staging.tfvars` *verbatim*, so whatever key sits in the committed example is what gets authorized for `root` on **your** droplet. It ships as a placeholder (`ssh-ed25519 AAAA...replace-with-your-own-public-key`), which authorizes nobody — replace it with your own key, or set `[]`, and edit `staging.tfvars.example` itself (`staging.tfvars` is gitignored and would be overwritten by that `cp`). See [Operator keys](/operate/ssh-access/#operator-keys).
