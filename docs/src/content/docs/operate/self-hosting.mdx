...
- **Nothing is encrypted at rest, and only `/supervisor` has a second wall.** OAuth tokens and secrets are plaintext columns. The admin panel that renders them sits behind an HTTP Basic realm keyed on `SUPERVISOR_PASSWORD`, which fails closed — unset or blank means every dashboard returns 401, so you have to set it before the panel opens for you either. That is one shared credential in front of one panel, not a login system. The rest of the app authenticates nobody, so the tailnet perimeter is still the wall. Don't expose port 80. See [Auth overview](/auth/overview/).
