...
2. **Point the fork at yourself.** The repo is Zimmer's own deployment, so a handful of values are hardcoded to `tadasant`. A fresh fork **will not deploy** until you rewrite them — the image build pushes to `ghcr.io/tadasant/…`, which your `GITHUB_TOKEN` has no rights to. Change these and commit:
   ...
   | `config/deploy.staging.yml` | `APP_HOST` and `AGENT_ORCHESTRATOR_STAGING_BASE_URL` → your host. Terraform's `domain` only turns on the TLS terminator; **it does not feed Rails.** Leave these and every generated URL and OAuth callback points at someone else's host. |
...
1. FIRST, repoint the fork at me — a fresh fork does NOT deploy until you do. The repo is Zimmer's own deployment and hardcodes 'tadasant' in several places; the image build would push to ghcr.io/tadasant/... which my GITHUB_TOKEN cannot write to, and it would 403. Ask me for my GitHub username/org and my host, then edit and commit:
   ...
   - config/deploy.staging.yml: APP_HOST and AGENT_ORCHESTRATOR_STAGING_BASE_URL -> my host. (Terraform's 'domain' only turns on the TLS terminator; it does NOT feed Rails. Leaving these points every generated URL and OAuth callback at someone else's host.)
...
- **SSH is open to `0.0.0.0/0`.** The firewall allows `22/tcp` from anywhere and there's no variable to narrow it.
