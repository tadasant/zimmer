...
- **🔴 Set `admin_ssh_pubkeys` before your first deploy.** `deploy-staging.yml` does `cp staging.tfvars.example staging.tfvars` *verbatim*, so whatever key sits in the committed example is what gets authorized for `root` on **your** droplet. It does **not** ship a placeholder: it ships the upstream author's two real public keys (`zimmer-production-operator` and `agent-orchestrator-prod-hetzner`). Deploy it unchanged and you have authorized *him* for root on *your* box. Replace both with your own key, or set `[]`, and edit `staging.tfvars.example` itself (`staging.tfvars` is gitignored and would be overwritten by that `cp`). See [Operator keys](/operate/ssh-access/#operator-keys).
