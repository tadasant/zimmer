...
- An **Anthropic account** for Claude Code, and/or an **OpenAI account** for Codex. Like the GitHub login above, this is an OAuth flow you run inside the container after the first deploy (`claude /login` / `codex login`, [step 10](#stand-it-up)). Both persist across deploys: Claude's credentials on the `claude_home` volume, Codex's on `codex_home` (which also carries its rollout transcripts and thread store). See [agent harness credentials](/auth/harness/).
...
10. **Log the instance in — Git first.** The app is up, but it can't do anything useful yet: an agent that can't authenticate to GitHub can't clone, can't push a branch, and can't open a PR. These are interactive OAuth device flows, so they can't be baked into a secret — you run them once, by hand, inside the container:
    ...
    This is **once, not per deploy**: `gh`'s token lands in `~/.config/gh`, Claude's in `~/.claude`, and Codex's in `~/.codex` — all durable named volumes (`gh_config`, `claude_home`, `codex_home`) shared by `web` and `worker` and re-attached across Kamal container swaps. Confirm with `gh auth status` and `claude whoami`; the UI surfaces the same state (`CliStatusService` refreshes it every two minutes).
