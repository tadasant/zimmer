...
- **Nothing is encrypted at rest, and the second wall covers two surfaces.** OAuth tokens and secrets are plaintext columns. The admin panel that renders them, and the mutating `POST /health/*` maintenance actions, both sit behind an HTTP Basic realm keyed on `SUPERVISOR_PASSWORD`, which fails closed — unset or blank means those return 401, so you have to set it before either opens for you. That is one shared credential in front of two surfaces, not a login system. The rest of the app authenticates nobody, so the tailnet perimeter is still the wall. Don't expose port 80. See [Auth overview](/auth/overview/).
