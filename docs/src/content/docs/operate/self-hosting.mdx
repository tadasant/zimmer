...
### The agent's brain: an account for at least one runtime

Zimmer runs coding agents, so it needs at least one of its [three runtimes](/sessions/runtimes/) credentialed. Without this, sessions spawn but no agent runs.

- An **Anthropic account** for Claude Code, and/or an **OpenAI account** for Codex. Like the GitHub login above, this is an OAuth flow you run inside the container after the first deploy (`claude auth login` / `codex login`, [step 10](#stand-it-up)). Both persist across deploys: Claude's credentials on the `claude_home` volume, Codex's on `codex_home` (which also carries its rollout transcripts and thread store). See [agent harness credentials](/auth/harness/).
- Or an **OpenRouter API key** for Pi, which needs no login at all. Set `OPENROUTER_API_KEY` on the Inference page's Pi tab after the first deploy — it goes to the Parameter Store, so it survives a container rebuild without a shell. Pi's own `pi_home` volume carries its settings; its transcripts live in each session's clone rather than there. See [Runtimes](/sessions/runtimes/#credentials).
...
- **`API_KEYS`** (`STAGING_API_KEYS`): comma-separated keys for the REST API and `/mcp`. The first one is the key your agent sessions are handed, and the deploy fails if it is empty. More keys can be minted later on `/settings/api_keys`.
...
6. **Write your tfvars.** Edit `infra/terraform/staging.tfvars.example` **and commit it** — set `region`, `droplet_size`, `domain` (or `""` for MagicDNS only), and `managed_db_cluster_name`. Terraform only provisions the host here; the app image, env, and data stores are Kamal's.
   ...
   The one thing that does **not** go in that file is `admin_ssh_pubkeys` — because the file is committed, and a key in it would be authorized for `root` on every fork's droplet. Set it as the `ADMIN_SSH_PUBKEYS` Actions **variable** instead (`gh variable set ADMIN_SSH_PUBKEYS -b '["ssh-ed25519 AAAA… you@laptop"]'`), or leave it unset and get `[]`. See [Operator keys](/operate/ssh-access/#operator-keys).
...
10. **Log the instance in — Git first.** The app is up, but it can't do anything useful yet: an agent that can't authenticate to GitHub can't clone, can't push a branch, and can't open a PR. These are interactive OAuth device flows, so they can't be baked into a secret — you run them once, by hand, inside the container:
    ...
    This is **once, not per deploy**: `gh`'s token lands in `~/.config/gh`, Claude's in `~/.claude`, and Codex's in `~/.codex` — all durable named volumes (`gh_config`, `claude_home`, `codex_home`) shared by `web` and `worker` and re-attached across Kamal container swaps. Pi has no login step here at all; its key is set from the UI, and `~/.pi/agent` is a durable volume (`pi_home`) on the same terms. Confirm with `gh auth status`, and read the Claude Code tile on the UI's CLI status page for the rest (`CliStatusService` refreshes it every two minutes). Do **not** reach for `claude whoami` — `whoami` is not a subcommand, so the CLI bills the word as a prompt and answers it with a whole agent turn.
...
9. Hand the Git login back to me. The instance needs its own GitHub auth before any session can clone a repo, push a branch, or open a PR — and it is an interactive OAuth device flow inside the container, so you cannot do it (you have no shell on the droplet, and there is no GH_TOKEN env var to set). Tell me to run, once:
   ...
   Explain that the gh, claude, and codex tokens persist on the gh_config, claude_home, and codex_home volumes, so all three logins are one-time and not per-deploy. Until I do the gh login, every session fails at the clone step even though /up is green. Do not call the setup complete until I confirm 'gh auth status' succeeds inside the container.
...
- **Nothing is encrypted at rest, and the second wall covers four surfaces.** OAuth tokens and secrets are plaintext columns. The admin panel that renders them, the API keys page, the mutating `POST /health/*` maintenance actions, and minting [console login tokens](/auth/overview/#the-agent-login-primitive-console-login-tokens) (itself off unless `CONSOLE_LOGIN_ENABLED=true`) all sit behind an HTTP Basic realm keyed on `SUPERVISOR_PASSWORD`, which fails closed — unset or blank means those return 401 (the console-login endpoints answer 403 first while their own flag is off), so you have to set it before any of them opens for you. That is one shared credential in front of four surfaces, not a login system. The rest of the app authenticates nobody, so the tailnet perimeter is still the wall. Don't expose port 80. See [Auth overview](/auth/overview/).
...
- **Your fork authorizes no operator key until you set one.** `deploy-staging.yml` does `cp staging.tfvars.example staging.tfvars` *verbatim*, so any key committed in that example would be authorized for `root` on **your** droplet. It therefore contains none: the list comes from an `ADMIN_SSH_PUBKEYS` repository variable, and a fork that never sets it deploys with `admin_ssh_pubkeys = []`. That is a working droplet — Kamal deploys with its own key and you reach the box over Tailscale SSH on `:22` — but the publickey door on `:2222` has nothing on it, so an `ssh-*` MCP server in a session cannot attach. To get it, set the variable (Settings → Secrets and variables → Actions → Variables) to a JSON list of **your** keys: `["ssh-ed25519 AAAA… you@laptop"]`. See [Operator keys](/operate/ssh-access/#operator-keys). If you already deployed from an older example that shipped keys, this does not evict them: `ignore_changes = [user_data]` means they stay in that droplet's `authorized_keys` until you redeploy with `recreate_droplet: true` or prune the file by hand.
