...
because the "Build base image" workflow has not run since a `Dockerfile.base` change. The two
paths answer different questions.
...
`bin/preinstall-mcp-packages` — into a content key, and looks for
`zimmer-base:content-<key>`. Absent means that exact declaration has never been published, so it
builds and pushes the base itself before building the app image FROM that tag. Present means reuse.
So a `Dockerfile.base` change reaches production on the merge that lands it, automatically, and a
base build that *failed* cannot be skipped by a later commit — the content tag is still absent.
...
`release-image.yml` will never notice it; the monthly cron (`0 6 1 * *`) and manual dispatch exist for
exactly this. It publishes `content-<key>` **and** `:latest`, computing the key the same way
`release-image.yml` does — a refresh published anywhere else is one the app image never picks up,
which is what happened for every refresh between content addressing landing (2026-09-01) and this
being fixed. It builds uncached for the same reason: a cache hit would replay the layers it exists to
renew.

Two guards keep it honest. Only `refs/heads/main` may publish the shared tags — a dispatch from a
branch parks its build under `zimmer-base:branch-<ref>`, so the workflow is safe to exercise from a
pull request. And after the push, the run pulls the tag back down and asserts that every `name@version`
`Dockerfile.base` pins is resolvable inside the published image
(`.github/scripts/verify-base-image.sh`). `test -f` inside a Dockerfile says a layer was built; only
this says the tag the release path resolves actually serves it.
