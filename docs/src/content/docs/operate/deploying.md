...
| 3 — **contract** | `remove_column` on the ten dead names, annotated `# two-phase-drop: phase 2 of #1179` — merged only once deploy 2 was live | the `ignored_columns` line goes |
...
| Surface | Where |
| --- | --- |
| **Health page panel** | `/health` → **Post-Deploy Tasks** — status per task, counters, the error text, and a **Re-arm and run now** button. Like the rest of the web UI, neither the panel nor the button asks for a credential |
