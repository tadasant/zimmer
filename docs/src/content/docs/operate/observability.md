...
restart — as one uniform section, rendered on `/health`, returned by `GET /api/v1/health`, and included in the
...
loop: seven of them fail the session, and the empty-turn restart parks it in `needs_input`
...
answerable for all eight loops. It was answerable for two of them until #527 — the section
