...
8. If the service resolves secrets from a Google Parameter Store, set its resolver key as a
   GitHub Actions secret here (for Zimmer,
   `PROD_ZIMMER_PARAMS_RESOLVER_SERVICE_ACCOUNT_KEY_JSON`) and export it in the deploy job.
   **Base64 of the key JSON, not the JSON** — a Docker env-file carries no newline, and the key
