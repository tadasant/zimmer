...
| daily 04:25 | `InboundEventRetentionJob` | Delete `webhook_deliveries` rows older than 7 days and `trigger_event_claims` rows older than 30 — the dedup stores behind [Slack Events API delivery](/sessions/triggers/#slack-events-api-delivery). Batched and idempotent, on `maintenance`. |
