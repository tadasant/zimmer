...
| 10m | `TokenUsageIngestionJob` | Sweep recent transcripts into the token-spend ledger, once per runtime that has a `usage_ingestor_class` in `RuntimeRegistry` — a filesystem scan for Claude Code, a `sessions.transcript` read for Pi. Looks only at transcripts touched in the last two hours; the lookback overlaps the interval generously because ingestion is idempotent on `request_id`, so a missed run closes itself on the next pass. One ingestor failing is logged at `error` and does not cost the others their sweep. History that predates the job is swept once — by `TokenUsageBackfillJob` for Claude Code, by a one-time post-deploy task for Pi. See [Token spend](/operate/costs/). |
