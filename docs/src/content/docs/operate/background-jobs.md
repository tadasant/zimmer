...
| 1m | `OutcomeAnalysisBatchPumpJob` | Advance every running Outcomes "Analyze All" batch: reconcile in-flight analyses, spawn the next wave. A stopped batch with analyses still in flight is reconciled too, and spawns nothing |
...
| 5m | `StrandedSleepSweepJob` | Resume a session asleep in `waiting` on a wake-up that can never fire. The other half of the hole `StalledStartSweepJob` covers: that one is for a session whose **first** turn was lost, this one for a session that ran, went to sleep on a `wake_me_up_later` / `wake_me_up_when_session_changes_state` wake, and lost it — production session 6412, a router orchestrating several children, sat there for 38.7 hours ([#855](https://github.com/tadasant/zimmer/issues/855)). The predicate is **no fireable wake**, not *no wake*: a watcher whose watched session is already archived is an `enabled` row that can never fire, and a sweep keyed on the absence of trigger rows would walk straight past it. It reads the converse too, which it did not always: a one-time schedule that came due within `SessionStateMachine::SCHEDULE_FIRE_SETTLE` (10 minutes), and a `session_archived` watcher whose watched session archived that recently, are wakes still *in flight* — `ScheduleTriggerJob` is a one-minute cron and `AoEventTriggerJob` is enqueued after the watched transition commits. Without that, a wake set for a round five-minute boundary raced this sweep's own tick: session 13229's wake came due at 11:20:00, the sweep alerted that it could never fire at 11:20:08, and it fired at 11:20:18 into a session the rescue had already resumed. Requires a `session_id` (a session that never ran belongs to the sweep above), no pending enqueued message, no unfinished `AgentSessionJob` (an anti-join), none of the dormant markers, and `updated_at` quiet for `StrandedSleepRescue::GRACE` (15 minutes). It **pages** rather than taking one `ORDER BY updated_at LIMIT n`: fireability cannot be a `WHERE` clause, and a legitimate sleeper does not advance `updated_at` while it sleeps, so a single limited page would fill with wakes set days out and hide every stranded session behind them — permanently, while logging that it found none. It walks a total `(updated_at, id)` cursor up to `MAX_EXAMINED_PER_SWEEP` (1000), asking both Ruby-side predicates once per page. Resumes through `claim_system_recovery_turn!` with a `SYSTEM_RECOVERY` nudge, bounded at `MAX_ACTIONS_PER_SWEEP` (5) a pass and `MAX_RESCUES` (3) per session — past that it alerts and stops rather than resuming forever. A **status-summary fork** in this population takes the other repair. It arrives in the same shape — the turn `SessionStatusSummaryGenerator` dispatched to it went missing — but `AgentSessionJob` refuses any turn a fork is handed that is not the summary request, so resuming one spends the budget on a turn that cannot run and writes the ERROR record that pages `#alerts` about a fork behaving as designed ([#1168](https://github.com/tadasant/zimmer/issues/1168)). The sweep enqueues `SessionStatusSummaryHarvestJob` instead — the disposal that refusal reaches anyway, because `waiting` is the one resting state that harvests nothing — counts it `harvested` rather than `rescued`, and spends the same `MAX_RESCUES` budget — a harvest that will not take is abandoned and alerted on the fourth sighting rather than re-enqueued every `GRACE` forever. `AbandonedStatusSummaryForkSweepJob` owns the fork that was never dispatched, and excludes this one by construction: `running_job_id` and `pending_follow_up_prompt` are both stamped by `Session#deliver_follow_up!` and neither is cleared when the job is lost. Production and staging only — the repair spends a turn. See [Lifecycle](/sessions/lifecycle/#who-else-moves-sessions-around). |
| 1h | `WorkBacklogLivenessSweepJob` | Re-check [work backlog](/operate/work-backlog/) rows that have LEFT `queued` — a `started` row whose session ended more than `WorkBacklog::LivenessSweep::GRACE` (6 hours) ago, or one `removed` on a reason with a shelf life (`issue_has_open_pr`, `session_already_working`) — and record what GitHub now says in `liveness_state`. Before it, nothing read such a row again, so an expired premise left the item neither queued nor worked: 41 of 159 open convergent issues on 2026-09-11, 13 of them for four days or more. It **changes no row**. A merged PR with no closing keyword means either the work is finished or a deliberate remainder is outstanding, and separating those took reading each PR's scope and the current code — judgement per issue, not a rule — so it classifies and a human or agent triages. Evidence comes from the issue timeline (`Github::IssueLinkProbe`, one GraphQL request per 40 issues), which unlike search's `linked:pr` sees non-closing references and each PR's last activity, so an open PR idle for `STALE_PR_AFTER` (14 days) reads as `pr_stalled` rather than in flight. A failed read is `unknown`, never a conclusion. Bounded at `MAX_EXAMINED_PER_SWEEP` (200) a pass, taking rows whose verdict can still change before settled ones (`issue_closed`, `superseded`), because the candidate population only grows and closed rows would otherwise use up the budget. Logs at WARN only when a repo could not be read ([#584](https://github.com/tadasant/zimmer/issues/584)); the standing count is the **Stranded** section of the [Issues view](/operate/issues-view/) and `counts.stranded`, and it alerts when the oldest unresolved row passes `ALERT_AFTER` (7 days). A row a triager [held for a human decision](/operate/work-backlog/#held-for-a-human-decision) is out of the alert until its hold lapses (14 days) or a pass reads different evidence on it, which voids the hold. Production and staging only — it spends GitHub API budget. |
...
| 10m | `TranscriptArchiveJob` | Rebuild `latest.zip` under `~/.zimmer/transcript_archives` — the `zimmer_data` volume both roles mount, **not** `Rails.root/storage`. Cron runs in the `worker` container and every reader of the archive is an HTTP route in `web`, so a path on the container layer would be invisible to the reader and wiped by each deploy ([#714](https://github.com/tadasant/zimmer/issues/714)). Override with `AGENT_TRANSCRIPT_ARCHIVE_DIR`, onto a path both roles share. Reads one session at a time and archives at most `MAX_SESSIONS_PER_RUN` per tick, checkpointing the slice it finished — loading every changed session at once is what OOM-killed the production worker every ten minutes ([#719](https://github.com/tadasant/zimmer/issues/719)). Change detection compares the later of `sessions.updated_at` and the session's newest subagent-transcript `updated_at`, because a session's zip entry carries both and `SubagentTranscript` does not `touch:` its parent ([#720](https://github.com/tadasant/zimmer/issues/720)). A run with a backlog logs `deferred to the next tick` at WARN. Each tick first sweeps the `latest_<hex>.zip.tmp*` files that killed runs orphaned — the `ensure` that deletes the build's own temp does not survive a `SIGKILL`, and 219 of them reached 233.3 GiB and filled production's root filesystem ([#1160](https://github.com/tadasant/zimmer/issues/1160)). The pattern reaches rubyzip's sibling temp as well as the job's, spares anything written in the last hour, and cannot match `latest.zip` or `latest_metadata.json` |
| 10m | `TokenUsageIngestionJob` | Sweep recent transcripts into the token-spend ledger, once per runtime that has a `usage_ingestor_class` in `RuntimeRegistry` — a `~/.claude/projects` scan for Claude Code, a `sessions.transcript` read for Pi, a `~/.codex/sessions` rollout scan (Zstandard-decompressing the finished ones) for Codex. Looks only at transcripts touched in the last two hours; the lookback overlaps the interval generously because ingestion is idempotent on `request_id`, so a missed run closes itself on the next pass. One ingestor failing is logged at `error` and does not cost the others their sweep. History that predates the job is swept once — by `TokenUsageBackfillJob` for Claude Code, by a one-time post-deploy task for Pi and for Codex. See [Token spend](/operate/costs/). |
...
| daily 04:25 | `InboundEventRetentionJob` | Delete `webhook_deliveries` rows older than 7 days and `trigger_event_claims` rows older than 30 — the dedup stores behind [Slack Events API delivery](/sessions/triggers/#slack-events-api-delivery). Batched and idempotent, on `default`: a day's expiry is a sub-second run of indexed deletes. |
...
| 15m | `QuotaResetCheckerJob` | Restore `quota_exceeded` Claude accounts (on a fresh probe) and Codex accounts (on the rate-limit reading Codex recorded when it refused them); fire the `quota_available` event if that was the pool's rising edge AND no quota window is holding spot work at its utilization limit (which spawns one fleet-maintenance session to wake spot work in precedence order — a rising edge against a window-held gate is deferred to the next sweep rather than spent); then resume the parked sessions `AuthOutageWakeAuthority` gives the sweep — the **priority** ones — at most 5 per runtime per sweep, oldest park first, so a recovered pool is not re-drained by the whole cohort at once; and ask for a fleet wake on behalf of the **spot** parks it left alone, which is what keeps a park the fleet wake never reached from waiting for the pool to exhaust and recover again. See [The Claude Code harness](/auth/harness/). |
...
| 15m | `ClaudeUsageSamplerJob` | Read the quota of the accounts the spot gate decides on, so it decides on a fresh number — `QuotaResetCheckerJob` samples only *exceeded* accounts, and a healthy one is otherwise read only when somebody opens /inference. The **serving** account every tick, plus up to `MAX_SPARE_PROBES_PER_TICK` (2) other accounts whose newest reading is older than `SPARE_MAX_STALENESS` (60 minutes), stalest first. Sampling on staleness rather than on a fixed rotation means every other writer counts: an exceeded account the reset checker just re-read, or one somebody just looked at on /inference, is never stale enough to be picked. `96 + 24(N-1)` probes a day for a pool of N while its probes answer, and never more than five in one tick — a probe that fails writes no reading, so that account stays eligible and a second budget bounds how many refusals one tick walks past. See [Spot and priority](/sessions/spot-and-priority/). |
| 15m | `RefreshXOauthTokensJob` | Refresh X/Twitter tokens. X rotates refresh tokens single-use, so the job splits network failures: a connection that never established (`Net::OpenTimeout`, `ECONNREFUSED`) left the token unspent and is retried in-band with backoff, while a failure after the request may have gone out (`Net::ReadTimeout`, `ECONNRESET`) is deferred to the next scheduled run rather than re-sending a token X may already have consumed. Both classifications depend on the token request being bounded: `XOauthCredential::TOKEN_REQUEST_TIMEOUT` (10 seconds) caps the connect and the read, so a token endpoint that accepts the connection and then goes silent fails fast instead of holding a `default`-queue thread for as long as it likes. A permanent refusal clears the refresh token, and no job can recover from that: an operator re-authorizes from the credential's page in `/supervisor` ([how](/auth/mcp-oauth/#x-twitter-is-minted-from-supervisor)). |
| 30m | `RefreshMcpOauthTokensJob` | Refresh MCP OAuth tokens expiring within the hour, after reconciling each against every runtime's **host-global** on-disk store — Pi's and Codex's; Claude Code's is per session and the injector reconciles it at every spawn instead (see [MCP OAuth](/auth/mcp-oauth/#capturing-the-token-the-runtime-rotates-write-back)) — the Pi store is probed out-of-process, so this job spawns a short-lived `node` helper per distinct server it reconciles |
...
reading and editing while the gems land — but only for a clone that actually needs an install. The
case where it does not is settled inline, before the agent process exists; see **Most clones never
install anything** below. Two properties of the job itself are load-bearing, and both come from
...
Because that path does no work, it does not wait for a queue either. `AgentSessionJob` calls
`BundleInstallJob.adopt_image_bundle_now` while it is setting the clone up, and enqueues the job only
when the answer is "this clone needs gems of its own". The queue was the whole of
[#592](https://github.com/tadasant/zimmer/issues/592): `CliSpawnEnv` strips every `BUNDLE_*` and
...
no clone root is a normal state that lasts minutes. It is not silent either way: the follow-up path
rebuilds the root from the session row as a matter of course, the resume path does too where it safely
can ([a clone that vanished is rebuilt, not fatal](/sessions/spawning/#a-clone-that-vanished-is-rebuilt-not-fatal)),
both say so on the session's own timeline, and a resume whose rebuild is refused still fails with
"clone directory not found". Worth knowing what that costs in signal: a live clone root going missing
stays deliberately unreported by this job, and on the resume path it is now a session-log line and a
`lost_clone` row on the retry-budget health section rather than a failed session, until the budget runs
out.
...
Timeout.timeout(10) { Open3.capture3(binary, "--version") }
```

`Open3.capture3` runs its body inside `Open3.popen_run`, whose `ensure` closes the parent pipe ends
and then calls `wait_thr.join`. When `Timeout::Error` is raised in the calling thread it unwinds
*through that ensure*, and the join blocks until the child exits on its own. So the call takes
exactly as long as the child takes; all the timeout does is replace the return value with an
exception **after** the wait. Measured on Ruby 3.4.6, a 1-second timeout around an 8-second
SIGTERM-ignoring child returned after 8.03s. It also dumps two thread-death backtraces to stderr on
every fire (`IOError: stream closed in another thread`, from the two reader threads `capture3`
spawns) — unactionable noise in worker logs.

Seven call sites believed they had a wall-clock limit and did not
([#908](https://github.com/tadasant/zimmer/issues/908)). Two of them sat on the `waiting → running`
session launch path, which is the hazard `BoundedSubprocess` was written for in the first place:
...
| Call | Bound |
| --- | --- |
| `AirPrepareService::AIR_VERSION_PROBE_TIMEOUT_SECONDS` — `air --version` | 10s |
| `DockerComposeCleanupService::COMPOSE_DOWN_TIMEOUT` — `docker compose down -v --remove-orphans` | 120s |
| `ClaudeCodeUpdateJob::UPDATE_TIMEOUT` — `claude update` | 120s |
| `ClaudeCodeUpdateJob::VERSION_TIMEOUT` — `claude --version` | 30s |
| `CliStatusService::VERSION_TIMEOUT` — each configured `check_version` command | 10s |
| `CodexRuntimeAdapter::VERSION_TIMEOUT` — `codex --version` | 10s |
| `PiRuntimeAdapter::VERSION_TIMEOUT` — `pi --version` | 10s |

Two neighbours look like the pattern and are not it, so leave them alone.
`NativeClaudePrintRunner` wraps `ProcessManager#wait(pid)` — not `capture3` — and its
`rescue Timeout::Error` kills the pid, which is the legitimate spawn-then-wait-then-kill shape.
`CertExpiryChecker` wraps `ssl.connect`: in-thread IO, no child process.

**A guard keeps it from coming back.** `InertSubprocessTimeoutGuard`
(`test/support/inert_subprocess_timeout_guard.rb`) parses every Ruby file under `app/` and `lib/`
...
### A sweep that stops is noticed: cron freshness

Being a singleton has a cost. Before `SingletonSweep`, a sweep that wedged piled up ready copies and
the backlog page fired, which is how the 2026-08-22 incident was found. With `total_limit: 1`, one
unfinished row makes GoodJob refuse every later tick for that class, for ever, and the backlog stays
flat. Nothing the queue gates measure moves, so the sweep just stops
([#619](https://github.com/tadasant/zimmer/issues/619)). The same silence covers a key the cron
...
| Cadence | Grace | Why |
| --- | --- | --- |
| 30s to 15m | 30 minutes (`GRACE_FLOOR`) | a fast singleton that legitimately runs past a tick or two is not stopped |
...
| a copy ready and waiting for a worker | `overdue` | no. A queue that is not draining is what the lane thresholds, the wedged-lane branch and the Grafana `not draining` rule page on, each sized for its lane (an `agents` job waiting hours is admission control). Paging here too would say it twice, and would page right after [queue recovery mode](#queue-recovery-mode) lifts, while the lanes it froze are still draining |

Two readings are never judged: a key disabled in the GoodJob dashboard (`disabled`), and a key whose
entry in `config/cron_schedule.rb` carries `freshness_exempt:`, whose value is the sentence saying why
it may go silent for longer than its cadence (`exempt`). `CronSchedule.validate!` refuses the key
without a sentence, and `for` strips it before GoodJob sees the entry. No entry needs one today.

**The page.** `SystemHealthMonitorJob` reads `CronFreshness` on every run, and once a `stale` reading
...
directly rather than the rescued health section, so a check that cannot read fails the job loudly
instead of reading as "nothing stale".

What fires it, and why each is real:
...
halts `SystemHealthMonitorJob`, so the "Queue backlog critical" and "Cron schedule stale" pages stop
firing while the mode is on. That is deliberate, since the backlog is now the operator's own doing.
The singleton copies the halt froze wait for a worker once it lifts, and [cron
freshness](#a-sweep-that-stops-is-noticed-cron-freshness) reads a waiting copy as `overdue`, which
never pages.
...
| `maintenance` | 4 | Filesystem scans, `bundle install`, docker prune, transcript archiving — minutes each, same shape. The scheduled sweeps cap themselves at `SWEEP_BUDGET_SECONDS` ([above](#the-scheduled-sweeps-yield-the-maintenance-thread)); the package installs do not, and they are what the ceiling is sized for. Sized when the lane ran two threads and kept at four as a conservative bound | 100 | 60m |
...
- **Message granularity decides grouping.** Keep ids out of the message and in the context
  ...
  incidents genuinely want separate issues — `SystemHealthMonitorJob`'s three backlog titles and
  its "Cron schedule stale" are the worked example, and `WorkerWedgeAlert` names the host.
