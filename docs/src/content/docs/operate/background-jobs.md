...
| 15m | `QuotaResetCheckerJob` | Restore `quota_exceeded` Claude accounts; fire the `quota_available` event if that was the pool's rising edge AND the spot gate has room for a spot session (which spawns one fleet-maintenance session to wake spot work in precedence order — a rising edge against a held gate is deferred to the next sweep rather than spent); then resume the parked **priority** sessions directly — at most 5 per sweep, oldest park first, so a recovered pool is not re-drained by the whole cohort at once. See [The Claude Code harness](/auth/harness/). |
