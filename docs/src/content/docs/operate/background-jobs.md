...
| 1h | `WorkBacklogStaleStartSweepJob` | Re-check the [work backlog](/operate/work-backlog/)'s `started` rows whose session ended more than `WorkBacklog::StaleStartSweep::GRACE` (6 hours) ago, and put back the ones the fleet dropped. Before it, a row that reached `started` was never read again, so an implementing session that archived without closing its issue left the item neither queued nor worked, permanently — 41 of 159 open convergent issues on 2026-09-11, one of them for 37 days. The decision per row is **re-queue only when the session left nothing behind**: a closed issue marks the row finished (terminal); a PR the session merged, a PR it left unresolved, or a PR GitHub links to the issue (`linked:pr`, one search per repo) leaves it alone; only an open issue with none of those is re-queued, at the precedence it left with. A GitHub read that fails is `unknown`, never a re-queue. Bounded at `MAX_EXAMINED_PER_SWEEP` (200) and `MAX_REQUEUES_PER_SWEEP` (10) a pass, and `MAX_REQUEUES` (3) per item — past that it alerts and leaves the row. Logs examined/re-queued/outcomes and the age of the oldest stranded row every pass, at WARN when it re-queued something and INFO otherwise, so only a repair reaches the production log store ([#584](https://github.com/tadasant/zimmer/issues/584)); the standing count is the **Stranded** section of the [Issues view](/operate/issues-view/) and `counts.stranded` in `get_work_backlog`. Production and staging only — it spends GitHub search budget. |
