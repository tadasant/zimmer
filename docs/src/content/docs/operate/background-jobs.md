...
| 1m | `FleetIdleCheckerJob` | Fire the `no_sessions_in_progress` trigger event once the fleet has had nothing `running` and nothing spot-classified `waiting` for 5 continuous minutes. Idleness is a level, not an edge, so `FleetIdleMonitor` latches the fire: one event per quiet stretch, re-armed only when the fleet has work again. Production and staging only — the fire spawns a real session. See [Triggers](/sessions/triggers/#system_event). |
