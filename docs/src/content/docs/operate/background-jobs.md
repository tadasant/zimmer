...
| 30m | `RefreshMcpOauthTokensJob` | Refresh MCP OAuth tokens expiring within the hour, after reconciling each against **every** runtime's on-disk store (see [MCP OAuth](/auth/mcp-oauth/#capturing-the-token-the-runtime-rotates-write-back)) — the Pi store is probed out-of-process, so this job spawns a short-lived `node` helper per distinct server it reconciles |
