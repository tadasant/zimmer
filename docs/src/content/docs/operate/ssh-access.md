...
| Everything times out for a few minutes and then recovers on its own, while `tailscale status` says the node is `Online` | The host is thrashing on memory, which starves `tailscaled` of the data path. [Online is a control-plane fact](/limitations/#a-node-can-report-online-while-every-connection-to-it-times-out), not a reachability one | Look at the host, not at SSH: `dmesg -T \| grep -i "killed process"`, `free -m`, `uptime` |
