        ...
        <div data-trigger-form-target="systemEventConfig" class="hidden space-y-3">
          <div>
            ...
            <select name="${name}[configuration][event_name]" disabled class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md px-3 py-2 pr-8">
              ...
              <option value="no_sessions_in_progress">Fleet has room for more work (few enough sessions running or queued — rate-limited; tune on /inference)</option>
            </select>
            <p class="mt-1 text-xs text-gray-500">Deployment-wide events, each fired once on the edge rather than on every check. "Quota available again" fires when the Claude Code account pool goes from serving nothing to serving something, and is what wakes quota-parked spot sessions. "Fleet has room for more work" fires when the deployment has held fewer sessions than its configured ceiling — running ones and spot-queued ones counted together — for the whole of its configured stretch, with nothing parked on a quota outage and a pool that can serve. It cannot fire again until the fleet reaches that ceiling, and no more often than the cooldown allows, so it is a wake for a fleet with capacity to spare rather than one per check. All three numbers are set on the Backlog top-up card on /inference.</p>
          </div>
        </div>
