...
class ApiKey < ApplicationRecord
  ...
  class << self
    ...
    def authenticate(presented)
      ...
      api_key ||= register_env_key(presented, digest) if in_env
    ...
    def register_env_keys
      env_keys.each do |key|
        digest = digest(key)
        register_env_key(key, digest) unless exists?(token_digest: digest)
      end
    end
    ...
    def fingerprint_of(key)
      digest(key)[0, FINGERPRINT_LENGTH]
    end
    ...
    # today, so a failed write authenticates the key anyway, as an unsaved row with
    # the same name — which is exactly what `API_KEYS` did before this table existed.
    # `create_or_find_by` settles the race of several workers registering one key at
    # once.
    def register_env_key(key, digest)
      name = "#{ENV_NAME_PREFIX}#{digest[0, FINGERPRINT_LENGTH]}"
      create_or_find_by!(token_digest: digest) do |api_key|
        api_key.name = name
        api_key.source = ENV_SOURCE
      end
  ...
  def record_use!(now: Time.current)
    ...
    Rails.logger.warn("[api_key] could not stamp last_used_at on \"#{name}\" (id=#{id}): #{e.class}: #{e.message.truncate(200)}")
  end

  private

  def name_not_reserved
    return unless name.to_s.start_with?(ENV_NAME_PREFIX.strip)
