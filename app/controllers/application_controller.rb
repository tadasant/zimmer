class ApplicationController < ActionController::Base
  ...
  # Logged at INFO, on one line, with everything needed to answer "was this a bot
  # probe or a real user's stale form?". The session cookie is the discriminator:
  # present ⇒ a browser that has been here before (a stale form, an expired
  # session, an open tab across a deploy); absent ⇒ an unauthenticated probe.
  ...
  # session exists. Reading `session` would lazily mint an empty one and make
  # every request look session-present.
  def session_cookie_present?
    key = Rails.application.config.session_options[:key]

    key.present? && request.cookies.key?(key.to_s)
  end
