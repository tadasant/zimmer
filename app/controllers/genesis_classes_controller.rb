...
class GenesisClassesController < ApplicationController
  def update
    ...
    unless SessionGenesis.valid?(genesis)
      return redirect_to inference_path(anchor: "spot-gate"), alert: "Unknown genesis: #{genesis}"
    end
    unless SessionGenesis.settable?(genesis)
      return redirect_to inference_path(anchor: "spot-gate"),
    ...
    unless SessionGenesis::CLASSES.include?(klass)
      return redirect_to inference_path(anchor: "spot-gate"), alert: "Unknown class: #{klass}"
    end
  ...
  def destroy
    ...
    if setting.save
      redirect_to inference_path(anchor: "spot-gate"), notice: "Genesis classes reset to defaults."
    else
      redirect_to inference_path(anchor: "spot-gate"),
