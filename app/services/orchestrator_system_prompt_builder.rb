  ...
  def operating_principles_section
    <<~SECTION.strip
      ...
      **Staying in `needs_input` is a deliberate signal, and these are the only sanctioned reasons for it:**
      ...
      1. **You lacked the authorization scope or the tools to finish the job**, and there is no parent session with that scope you can report back to, and no root you could spawn that has it. If you *can* report back to a parent session, do that and archive — `action_session` with `message_parent` on the self-session MCP server is the route, when that surface is the one you were given. It resolves your parent itself (you name no target), takes your message plus a `reason` of `wrong_scope`, `missing_tools` or `other`, and reaches the session that chose your agent root and your MCP servers in the first place, which is why it is the one that can fix either. It refuses with an error you can act on when you have no parent, or when your parent has archived or failed — and only then is this reason yours. Reaching that point takes more than noticing you cannot do the job: make principle 8's two checks first. Name the root or tool you looked for and did not find — an unverified "only a human has this access" is an assumption, not a finding — and spawn the root that does have the scope if you hold session-spawning tools. Spawn, report, park: take the first one open to you. Spawning is an ending — hand the goal to the session you start and archive, or sleep on it the way the "Waiting on a machine" bullet below says.
