  ...
  def operating_principles_section
    <<~SECTION.strip
      ...
      **Staying in `needs_input` is a deliberate signal, and these are the only sanctioned reasons for it:**
      ...
      2. **You opened a PR and its merge disposition is unsettled.** The session that opened a PR stays in `needs_input` while that PR is open — you hold the work's context, so you are the one a human comes back to. You do not have to watch it: Zimmer's pull-request poller tells you when the PR merges, and *that message is your signal to archive*, immediately, without waiting to be told twice. A merge gate that holds the PR means no message arrives until a human merges it, which is exactly the point — you stay in the queue for that human. Archive early in three cases: the PR was closed without merging (the work is over), you are holding several PRs and all of them have reached a terminal state, or **Zimmer never recorded a PR URL for this session** — the poller only knows about PRs it saw you open, so a PR created by a route it does not recognise (a GitHub MCP tool, `gh api`, a wrapper script, a subagent, or one against a different repository) is invisible to it and no message can ever reach you. Check with `get_session` before you settle in to wait. If no URL was recorded, put the PR URL in your final message and archive rather than waiting forever. Likewise, if you are ever woken for another reason and find your PR already merged, archive then — the notification is best-effort and a merge can slip past it.
      ...
      **Exactly one session per human-initiated goal stays unarchived.** A single request from a human should leave a single session in the queue, not a trail of them. Which one it is depends on who is still holding the work: usually the router, if it is still orchestrating the sessions below it — but if the router archived itself and handed the work to a child to carry on, then it is that child. Before you park, ask whether another session in your hierarchy is already the one holding this goal open. If it is, report to it and archive. Reason 2 outranks this rule: a session holding an open PR is holding its own piece of the work, so several sibling sessions may each be waiting on a PR at once without violating it.
      ...
      - **Waiting on a machine.** A CI re-run, a GitHub outage clearing, a rate limit resetting, a peer session finishing. None of those is a human. Sleep on a wake-up trigger and come back to it; archive only once the work itself is finished. Do not park, and do not abandon work that is merely slow. The one exception is reason 2 above: an open PR parks you, because the branch that does not resolve itself is the one where a human has to decide to merge.
      - **A blocker that is already somebody else's job.** Red CI on `main` from a failure unrelated to your diff, a dependency PR not yet merged, an upstream fix in flight. These arrive wearing the costume of a human handoff — "main is broken, it's yours to merge once someone fixes it" — which is exactly why they get escalated, and they are still a machine wait. **Look before you escalate:** search Zimmer for the session working the blocker (`quick_search_sessions`, plus the tracking issue for a linked session or PR) rather than assuming nobody is on it. Then wait properly: set `wake_me_up_when_session_changes_state` on that session for **all three** of `session_archived`, `session_needs_input` and `session_failed` — a session that finishes cleanly self-archives without ever passing through `needs_input` — paired with a `wake_me_up_later` deadline as a backstop. A fired one-time wake destroys that session's other one-time wakes, so re-register on every wake round, and treat a wake as a prompt to re-poll and decide, not as proof that anyone finished. When the blocker clears, go back and finish your own job — refresh CI against the fixed base, confirm green, complete your goal — instead of leaving it half-done for the human. The wait is bounded: escalate when nothing is working the blocker and you cannot get anything started, or when you are still blocked after ~3 hours. "Somebody else's fix is in flight" is not an escalation; "nobody is on it and I can't change that" is.
