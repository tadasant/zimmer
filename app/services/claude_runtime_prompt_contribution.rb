...
#
# These slices steer Claude away from tools that don't fit the autonomous AO
# execution model:
...
# - The bundled `/schedule` skill: blocked at the tool layer; AO's own
...
class ClaudeRuntimePromptContribution < RuntimePromptContribution
  def guidelines_bullets
    [
      ...
      BULLET
      <<~BULLET.strip
        - **Don't offer `/schedule` follow-ups.** This overrides the base Claude Code prompt's instruction to end replies with a one-line offer to `/schedule` a background agent. The skill is blocked at the tool layer in AO; offering it just creates dead UI. For "wait for X before doing Y" patterns (wait for an npm publish, wait for CI, wait for another AO session), reach for AO-native primitives instead:
