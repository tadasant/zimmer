...
#
# Two things get reported, both into the session log the human reads:
#
#   * an OOM kill inside this session's cgroup, the moment the counter moves. This is
#     the one that matters, and the count is recorded on the session so that
#     ProcessLifecycleManager can tell "the agent died BECAUSE of the bound" from "the
#     agent died of something else, and a subprocess was killed an hour ago".
...
# (session ended, sweep ran) or a container with no cgroup support at all leaves this a
# no-op rather than an exception on the monitoring path.
class SessionMemoryWatch
  ...
  # Where the observed OOM-kill count is remembered between checks.
  ...
  # bound it was, because "your session hit its 4 GiB limit" is the sentence a bare
  # `Killed` in a tool result is missing.
  def report_oom_kills(stats, log_buffer)
    observed = stats.oom_kills
    return if observed.nil?

    previously_seen = @session.metadata[OOM_KILL_COUNT_KEY].to_i
    return if observed <= previously_seen

    killed = observed - previously_seen
    log_buffer.add(
    ...
    @session.merge_metadata!(OOM_KILL_COUNT_KEY => observed)
  end
