...
#
# Zimmer bounds nine distinct auto-recovery loops — SIGTERM retry, API-error retry,
# signal-death resume, MCP connection retry, context-length compact, session-id
# conflict recovery, empty-turn restart, silent recovery restart, lost-clone recovery.
# Each one counts its attempts in a `session.metadata` key, stamps when it last fired,
# has a maximum, and clears a set of keys once the process has been stable again for a
# while. Those four facts used to live in five different classes, in three naming
# conventions, with the reset logic written out three times and the metadata keys
# re-typed a fourth time as SQL inside HealthMonitorService.
#
...
# the monitoring loop resets and the health surface enumerates, so a tenth failure
