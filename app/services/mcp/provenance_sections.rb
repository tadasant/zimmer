  ...
  module ProvenanceSections
    ...
    def hierarchy_lines(hierarchy)
      ...
      if hierarchy.uncle_edges?
        lines << "A line marked `also senior: #N` carries an UNCLE edge: session #N queued or interrupted that session, so Zimmer treats #N as an additional parent — a sibling of the spawn parent — on the assumption that a session which inspected another and decided to redirect it holds information that session does not. Uncle edges are self-declared by the calling session, so read one as a claim of seniority rather than as proof of it. A wrong one is worth acting on rather than just noting: it widens the hierarchy human messages are gathered over, so it keeps pulling another lineage's context into both sessions until someone removes it. `action_session` with action `remove_uncle`, `session_id` of the junior and `uncle_session_id` of #N detaches exactly that edge."
      end
