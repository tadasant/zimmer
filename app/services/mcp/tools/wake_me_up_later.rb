    ...
    class WakeMeUpLater < Tool
      ...
      def self.rendered_description
        <<~DESC
          ...
          **A follow-up does not cancel your wake.** If somebody sends this session a prompt before the scheduled time — a router's `follow_up`, a human's message, a queued message draining, a Slack or GitHub trigger — the session takes that turn and the wake stays armed, and once you have answered the session goes back to sleep on it — you rest in `waiting`, not `needs_input`, and the wake fires on its own schedule (or is queued onto the turn if its moment arrives mid-turn). You do not need to re-register it. If answering that prompt changes your mind and you now need a human, say so in your answer: the wake still collects you, and you can stop arming wakes and rest in `needs_input` then. What DOES end the wake is a takeover: `restart`, a restart from scratch, or archiving the session — those replace the wait rather than adding to it.
