    ...
    class ActionTrigger < Tool
      ...
      description <<~DESC
        ...
        **Trigger types:**
        ...
        - **system_event**: Triggered when the DEPLOYMENT changes state, not a session. Broadcast and recurring, and every event is edge-fired — it fires on the deployment ENTERING the state, never once per check while it stays there. Two events: `{"event_name": "quota_available"}`, the account pool going from serving nothing to serving something, which is what wakes quota-parked spot sessions (there is normally exactly one such trigger and it is seeded by a migration — create another only deliberately); and `{"event_name": "no_sessions_in_progress"}`, the deployment having had fewer TURNS ON A WORKER than its configured ceiling for the whole of its configured stretch — `waiting` sessions never count, and neither does a turn queued behind the `agents` pool for a worker or a `running` row asleep on its own future wake (3 and 5 minutes by default, both tunable on /inference), nothing parked on an auth outage, and an account pool that can serve. Use it for work that should top up a fleet with capacity to spare; it deliberately stays quiet while the pool is empty or anything is parked on an outage, so it means "the fleet has room for more work", not "the gate is holding things". It fires at most once per quiet stretch AND at most once per cooldown (an hour by default), because the session it spawns would otherwise re-qualify it by running — with a ceiling above 1 that cooldown, not the ceiling, is what caps how often it fires. Pair it with `skip_if_pending_session` so a session it already spawned answers the next quiet stretch.
