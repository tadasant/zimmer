  ...
  module Tools
    ...
    class GetSession < Tool
      ...
      description <<~DESC
        ...
        **Session hierarchy:** the lineage graph this session belongs to — the origin session at the root and every descendant below it, each with its id, agent root and title. Indentation is the SPAWN edge and means "spawned", NOT "most recently talked to": a session is routinely followed up by a router other than the one that spawned it. A line ending `(shown under #N ...)` is one whose indentation is NOT a spawn edge — Zimmer reached it through an uncle, or the node ceiling placed it there — so read its position as "drawn here" rather than "spawned by the line above". A line marked `also senior: #N` is an UNCLE edge: session #N queued or interrupted that session and is therefore treated as an additional parent, which is why #N's hierarchy contributes `elsewhere` human messages below. Uncle edges are self-declared by the calling session — a claim of seniority, not proof of one. A wrong one can be detached with `action_session` → `remove_uncle` (`session_id` the junior, `uncle_session_id` #N); it otherwise keeps widening both hierarchies. Use the graph to see who spawned this session, who else is working the same goal, and therefore who it reports back to.
      ...
      def format_session_details(session, inline_transcript:, include_transcript:, verbose:)
        ...
        # A turn the starvation lane admitted past the gate, while the session
        # still carries the record of it. Said here rather than only in the
