  ...
  module Tools
    ...
    class PullWorkBacklogItems < Tool
      ...
      description <<~DESC
        ...
        **Two ways to say which.** `count` starts the top N by rank (0 is a legal, useful no-op that just re-ranks). `keys` starts exactly the items you name, in the order you give them, after you have read them with `get_work_backlog` and re-checked each on GitHub — that is the normal path, since **you must re-check before you start**: the item is a pointer to a live thread, and weeks can pass between an append and a pull. Live means open, no linked open PR, no unarchived session already working it; trusted means the thread's author and every commenter still pass the trust rules as it stands now. A key that is not queued fails the whole call. At most #{WorkBacklog::Pull::MAX} per call, so one bad night is bounded — how many to pull is your WIP arithmetic (`counts.in_flight` from `get_work_backlog` is the number of sessions this backlog produced that an agent is still advancing — `counts.parked`, the ones stopped in `needs_input` waiting on a person, is reported beside it and is deliberately not part of it — but a `parked` pile that keeps growing means the fleet is finishing work nobody is merging, and the useful action then is to get those PRs merged rather than to pull more; `get_work_backlog` with `status: "parked"` lists them). **Before you report a pull of zero, read `counts.spot_held`** — the subset of `in_flight` the spot gate is holding before a turn. Those items keep their WIP slot on purpose: they start themselves on their own re-check when the gate opens, so pulling more would only grow the pile that then resumes all at once. When most of `in_flight` is `spot_held`, call `get_spot_policy` for WHICH ceiling is holding before you call the run healthy: on `fleet_cap` every slot is taken and the fleet is busy, so zero is the healthy outcome; on `spot_budget` or `pacing_curve` the fleet is idle behind a quota window, and zero is still the correct pull but must not be reported as a healthy fleet.
        ...
        **Returns** JSON: `started` (each item with its `session` id, URL and precedence), `removed`, and the queue's `queued` / `in_flight` / `spot_held` / `parked` counts afterwards.
      DESC
