    ...
    class SearchGateDecisions < Tool
      ...
      description <<~DESC
        ...
        **Other shapes:**
        ...
        - `{ artifact_query: "pull/749" }` — the same column, matched as a case-insensitive substring. Use it when you have a fragment rather than the whole URL — a number, a path segment like `/issues/`, a repo name. Every row it returns is still a rating *of* that artifact.
      ...
      input_schema({
        ...
        properties: {
          ...
          artifact_query: {
            type: "string",
            description: "Substring over the same artifact URL, case-insensitive — for a fragment rather than the whole URL (\"pull/749\", \"/issues/\", a repo name). Narrower than `query`, which also matches the prose of the entry."
          },
          query: {
            type: "string",
