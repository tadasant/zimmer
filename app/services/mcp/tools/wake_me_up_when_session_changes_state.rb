  ...
  module Tools
    ...
    class WakeMeUpWhenSessionChangesState < Tool
      ...
      description <<~DESC
        ...
        **Fires on reaching the state, not on already being in it.** `failed` and `archived` are terminal — a session already in one will not enter it again — so this tool rejects those up front, along with the self-watch case, rather than letting the requester sleep on a trigger that can never fire. A watched session that is *already* resting in `needs_input` when you register fires immediately — with one exception. A session Zimmer parked to recover its own interrupted process (`paused_by: "recovery"`) is sitting in `needs_input` on its way back to `running`, and firing on it would hand you a session that has produced nothing to read while destroying the rest of your wake set. That case leaves the watcher armed for the real announcement, which is one more reason to pass all three events and a `wake_me_up_later` backstop rather than `session_needs_input` alone.
