  ...
  module Tools
    ...
    class ActionSession < Tool
      ...
      def restart_from_scratch(session)
        ...
        # created with. AgentSessionJob takes them only as job arguments, so
        # enqueuing bare re-ran the prompt with the screenshot silently missing
        # (#746). Everything on the volume is replayed deliberately: this path is
        # reached only for a pre-prompt failure with setup incomplete, and a
        # restart from scratch discards the conversation any earlier delivery went
        # to. Read outside the transaction; the read never raises, so the fleet
        # sweep's restart cannot start failing on an unreadable volume.
