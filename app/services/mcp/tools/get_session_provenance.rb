  ...
  module Tools
    ...
    class GetSessionProvenance < Tool
      ...
      description <<~DESC
        ...
        **Session hierarchy:** the origin session at the root and every descendant below it, each with its id, agent root and title, plus the graph's size and origin id. Indentation is the SPAWN edge and means "spawned", NOT "most recently talked to": a session is routinely followed up by a router other than the one that spawned it. A line ending `(shown under #N ...)` is one whose indentation is NOT a spawn edge — Zimmer reached it through an uncle, or the node ceiling placed it there — so read its position as "drawn here" rather than "spawned by the line above". A line marked `also senior: #N` is an UNCLE edge: session #N queued or interrupted that session, so Zimmer treats #N as an additional parent — a sibling of the spawn parent — on the assumption that a session which inspected another and decided to redirect it holds information that session does not. That is why #N's hierarchy contributes `elsewhere` human messages. Uncle edges are self-declared by the calling session — a claim of seniority, not proof of one. A wrong one can be detached with `action_session` → `remove_uncle` (`session_id` the junior, `uncle_session_id` #N); it otherwise keeps widening both hierarchies. Use the graph to see who spawned you, who else is working the same goal, and therefore who you report back to.
