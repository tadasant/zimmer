    ...
    class SelfSessionActionSession < ActionSession
      ...
      description <<~DESC
        ...
        **Archive guidelines:**
        ...
        - A session that opened a PR is the exception to the timing: stay in `needs_input` until that PR merges. Zimmer's pull-request poller reaches unarchived sessions only, and it delivers a message on the open → merged transition — that message is the signal to archive. A PR a merge gate holds never sends one, which is how the session stays in the queue for the human who must merge it
